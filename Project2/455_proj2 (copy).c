...
{
	struct arp_hdr* arp = (struct arp_hdr *)malloc(sizeof(struct arp_hdr));
	...
	byteSent = sendto(sockfd, buf, BUF_SIZ, 0, (struct sockaddr *)&sk_addr, &sk_addr_size);
	printf("\tDone\n");
	...
	arp = (struct arp_hdr*) &buf[14];
	
	printf("12\n");

	printf("%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", arp->ar_sha[0], arp->ar_sha[1], arp->ar_sha[2], arp->ar_sha[3], arp->ar_sha[4], arp->ar_sha[5]);
